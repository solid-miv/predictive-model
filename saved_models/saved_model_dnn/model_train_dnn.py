...
def build_evaluate_model(X_train, X_test, y_train, y_test):
    ...
    with open(os.path.join(os.getcwd(), "saved_models/saved_model_dnn/dnn_evaluation.txt"), "w") as file:
...
def save_model(model, format="keras"):
    model.save(os.path.join(os.getcwd(), f"saved_models/saved_model_dnn/dnn_model.{format}"))
